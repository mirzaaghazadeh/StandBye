...
import { agents, questions, runs, rules, spentToday, team } from "./data";
...
function Sidebar({ active }: { active: "home" | "inbox" | "runs" }) {
  const needs = questions.filter((q) => q.status === "open" && q.kind !== "report").length;
  return (
      ...
      <div className="app">
        ...
        <div className="main">
          <div className="tb">
            ...
            <Button icon={<Ic.Pause size={12} />}>Pause All</Button>
          ...
          <div className="body">
            <div className="split-v">
              <div className="th">
                <span style={{ width: 170 }}>Agent</span>
                <span style={{ width: 104 }}>Status</span>
                <span style={{ flex: 1 }}>Doing now</span>
                <span style={{ width: 96 }} className="mock-hide-sm">Model</span>
                <span style={{ width: 52 }}>Since</span>
              ...
              <div style={{ flex: 1, background: "var(--surface)" }}>
                {agents.map((a, i) => (
                  <div key={a.id} className={["tr", i % 2 === 1 && "tr-alt", a.id === selected.id && "tr-sel"].filter(Boolean).join(" ")}>
                    <span style={{ width: 170, display: "flex", alignItems: "center", gap: 8 }} className="cell">
                      <Avatar agent={a} size={22} />
                      <span className="cell"><b style={{ fontWeight: 500 }}>{a.name}</b><span style={{ color: "var(--ink-4)" }}> · {a.role}</span></span>
                    </span>
                    <span style={{ width: 104 }}><StatusPill status={a.status} /></span>
                    ...
                    <span style={{ width: 52, color: "var(--ink-4)", fontSize: 12 }}>{ago(a.lastRunAt)}</span>
            ...
            <div className="insp mock-hide-md">
              ...
              <Group title="RESPONSIBILITIES">
                {team.agents.find((d) => d.name === selected.name)?.responsibilities.map((r) => (
          ...
          <div className="status">
            ...
            <span>Work hours 08:00–20:00</span>
            <span className="grow" />
            <span>Team cap ${team.dailyCapUsd}/day</span>
          </div>
        </div>
      </div>
    </div>
          ...
          {runs.map((r, i) => {
            ...
            return (
              <div key={r.id} className={"tr tr-sm" + (i % 2 ? " tr-alt" : "")}>
                <span style={{ width: 120, display: "flex", gap: 6, alignItems: "center" }} className="cell"><Avatar agent={a} size={18} />{a.name}<span style={{ color: "var(--ink-5)", fontSize: 11 }}>{r.trigger}</span></span>
