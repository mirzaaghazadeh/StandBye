...
import { DOWNLOAD, GITHUB, agents, models, team } from "./data";
import { HomeMock, InboxMock, RulesMock, RunsMock } from "./Mock";

const MCP_CMD = `CREW_AGENT=kai CREW_PORT=<port> CREW_TOKEN=<token> \\
    ...
    <header className="nav">
      <div className="wrap nav-in">
        ...
        <nav className="nav-links">
          ...
          <a href="#keys">Pricing</a>
          <a href="#faq">FAQ</a>
        </nav>
        <div className="grow" />
        <a className="btn nav-gh" href={GITHUB} target="_blank" rel="noreferrer"><Ic.Branch size={12} />GitHub</a>
        <a className="btn btn-primary" href={DOWNLOAD}><Ic.File size={12} />Download for macOS</a>
      </div>
    </header>
  );
}
...
function Logo() {
  return (
    <svg width="22" height="22" viewBox="0 0 32 32" aria-hidden>
      ...
      <circle cx="11" cy="12" r="4" fill="#fff" />
  ...
  return (
    <section className="hero">
      <div className="wrap">
        ...
        <h1>A standing team of AI agents.<br />Working while you're away.</h1>
        <p className="lede">
      ...
      <div className="wrap wrap-wide">
        <HomeMock />
        <div className="caption">The real Home screen: who is working, who needs you, what each agent is doing, what today cost.</div>
      </div>
    </section>
  );
}
...
function Keys() {
  const p = [
    { name: "Claude", how: "Your Claude Code login, or an Anthropic API key.", main: models.anthropic.main, checkin: models.anthropic.checkin, note: "Full Claude Code harness via the Agent SDK." },
    { name: "OpenRouter", how: "One key, hundreds of models.", main: models.openrouter.main, checkin: models.openrouter.checkin, note: "Any tool-capable model. GLM 5.3 by default." },
  ];
  return (
    <section id="keys" className="sec-block">
      <div className="wrap">
        <h2>Your keys, your bill</h2>
        <p className="muted wide">Standbye is free and open source. There is no account and no server of ours in the loop. You connect the providers you already pay, set a cap, and see the cost of every run.</p>
        <div className="providers">
          {p.map((x) => (
            <div key={x.name} className="card prov">
              <div className="prov-h"><Ic.Lock size={14} /><b>{x.name}</b></div>
              <div className="muted" style={{ marginBottom: 12 }}>{x.how}</div>
              <div className="kv"><span className="k">Default</span><span className="v"><Pill mono>{modelLabel(x.main)}</Pill></span></div>
              <div className="kv"><span className="k">Check-ins</span><span className="v"><Pill mono>{modelLabel(x.checkin)}</Pill></span></div>
              <div className="fine" style={{ marginTop: 10 }}>{x.note}</div>
            </div>
          ))}
          <div className="card prov prov-price">
            <div className="prov-h"><Ic.Dollar size={14} /><b>What a day costs</b></div>
            <div className="price-row"><span>Quiet check-in</span><span className="mono">≈ $0.01</span></div>
            <div className="price-row"><span>Starter team, estimated</span><span className="mono">${team.estimatedDailyUsd.low}–{team.estimatedDailyUsd.high}</span></div>
            <div className="price-row"><span>Starter team, hard cap</span><span className="mono">${team.dailyCapUsd}</span></div>
            <div className="price-row"><span>Standbye</span><span className="mono">$0</span></div>
            <div className="fine" style={{ marginTop: 10 }}>Estimates are the builder's own for the starter team. Your cap is the number that matters, and the app stops at it.</div>
          </div>
        </div>
      </div>
    </section>
...
function JoinFromClaudeCode() {
  return (
    <section id="mcp" className="sec-block alt-bg">
      <div className="wrap">
        <div className="two">
          <div>
            <h2 style={{ marginTop: 0 }}>Join the team from Claude Code</h2>
            <p className="muted">The team tools are an MCP server. Register it in Claude Code and your own session becomes a teammate: it reads channels, posts, asks the owner and finishes runs as that agent. The same tools the agents use, nothing hidden.</p>
            <div className="tools">
              {["post_message", "ask_agent", "assign_task", "ask_user", "propose_hire", "remember", "learn_skill", "escalate"].map((t) => <span key={t} className="pill mono" style={{ background: "var(--surface)", border: "1px solid var(--border)" }}>{t}</span>)}
            </div>
          </div>
          <pre className="code"><code>{MCP_CMD}</code></pre>
        </div>
      </div>
    </section>
  );
...
function Faq() {
  const qa = [
    ...
    ["Which models can I use?", "Claude models through your Claude Code login or an API key, and anything tool-capable on OpenRouter. Each agent picks a provider, a main model and a cheap check-in model."],
    ...
    <section id="faq" className="sec-block">
      <div className="wrap wrap-narrow">
        ...
        <div className="cta cta-end">
          <a className="btn btn-primary btn-xl" href={DOWNLOAD}><Ic.File size={14} />Download for macOS</a>
          <Button lg onClick={() => window.open(GITHUB, "_blank")} icon={<Ic.Branch size={12} />}>Star on GitHub</Button>
        </div>
      </div>
    </section>
  );
