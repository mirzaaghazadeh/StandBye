...
export const runs: Run[] = [
  ...
  { id: "r3", agentId: "rex", trigger: { kind: "task", title: "Review #214", details: "", from: "ada", event: "pr" }, status: "needs_you", summary: "Reviewed #214: one flaky test, asked to merge", model: defaultModelsFor("openrouter").main, startedAt: minutesAgo(26), finishedAt: minutesAgo(23), costUsd: 0.12, inputTokens: 38_000, outputTokens: 2_100, baseHead: "9f31ab8c2d7e5a04b1c6f9d0e3a7b2c8d4e5f6a1", stepCount: 9, error: null, createdAt: minutesAgo(26) },
...
// Word for word what the Runs screen says (screens/Runs.tsx), so the site's table reads like the app's.
const agentName = (id: string) => (id === "user" ? "You" : agents.find((a) => a.id === id)?.name ?? id);
export function triggerLabel(t: Run["trigger"]): string {
  switch (t.kind) {
    case "heartbeat": return "Checked in";
    case "schedule": return t.name;
    case "mention": return `${agentName(t.by)} mentioned them`;
    case "task": return `Task from ${agentName(t.from)}`;
    case "answer": return "You answered";
    case "question": return `${agentName(t.from)} asked them`;
    case "escalated": return "Found work";
    case "manual": return "You messaged them";
