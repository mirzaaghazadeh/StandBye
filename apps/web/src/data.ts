...
export const team: TeamDraft = soloDevTeam(providers, "Navid", "the app");
...
const live: Record<string, Live> = {
  Ada: { status: "working", statusText: "Writing the standup plan for #general", spent: 1.42, since: 4, next: 30, running: true, memoryCount: 12 },
  Kai: { status: "working", statusText: "PR #218: retry budget check on the queue", spent: 2.05, since: 11, next: 30, running: true, memoryCount: 9 },
  Rex: { status: "needs_you", statusText: "Asked: merge #214 after the flaky test fix?", spent: 0.6, since: 23, next: 30, memoryCount: 5 },
  Sol: { status: "idle", statusText: "Docs current as of 14:10", spent: 0.08, since: 52, next: 68, memoryCount: 3 },
};
...
export const runs: Run[] = [
  { id: "r1", agentId: "ada", trigger: "cron", status: "running", summary: "Standup: read #backend, drafting today's plan", model: DEFAULT_MODELS.anthropic.main, startedAt: minutesAgo(4), finishedAt: null, costUsd: 0.31, inputTokens: 41_200, outputTokens: 1_900, stepCount: 6, error: null, createdAt: minutesAgo(4) },
  { id: "r2", agentId: "kai", trigger: "mention", status: "running", summary: "Fix retry budget check, add test, open PR", model: DEFAULT_MODELS.anthropic.main, startedAt: minutesAgo(11), finishedAt: null, costUsd: 0.88, inputTokens: 122_000, outputTokens: 6_400, stepCount: 19, error: null, createdAt: minutesAgo(11) },
  { id: "r3", agentId: "rex", trigger: "event", status: "needs_you", summary: "Reviewed #214: one flaky test, asked to merge", model: DEFAULT_MODELS.openrouter.main, startedAt: minutesAgo(26), finishedAt: minutesAgo(23), costUsd: 0.12, inputTokens: 38_000, outputTokens: 2_100, stepCount: 9, error: null, createdAt: minutesAgo(26) },
  { id: "r4", agentId: "sol", trigger: "heartbeat", status: "noop", summary: "Nothing new since 13:40", model: DEFAULT_MODELS.anthropic.checkin, startedAt: minutesAgo(52), finishedAt: minutesAgo(52), costUsd: 0.01, inputTokens: 6_800, outputTokens: 90, stepCount: 1, error: null, createdAt: minutesAgo(52) },
  { id: "r5", agentId: "kai", trigger: "heartbeat", status: "done", summary: "Merged the queue fairness PR, CI green", model: DEFAULT_MODELS.anthropic.main, startedAt: minutesAgo(95), finishedAt: minutesAgo(81), costUsd: 0.74, inputTokens: 98_000, outputTokens: 4_300, stepCount: 14, error: null, createdAt: minutesAgo(95) },
] as Run[];
