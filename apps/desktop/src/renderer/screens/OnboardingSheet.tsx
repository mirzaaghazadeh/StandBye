      ...
      {step === 1 ? (
        ...
        <div className="sheet-body" style={{ flexDirection: "column", padding: "18px 24px", gap: 14 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0, 1fr))", gap: 12, flex: 1 }}>
            <Choice icon={<Ic.Sparkle size={22} stroke="var(--accent)" />} title="Describe it" body="Say what you're building and what you need done. Claude drafts the roles, souls, channels and budgets; you review before anything is created." action="Describe…" disabled={!providers?.anthropic.hasKey} note={providers?.anthropic.hasKey ? undefined : "Needs an Anthropic API key"} onClick={() => store.openSheet({ kind: "builder", mode: "describe" })} />
