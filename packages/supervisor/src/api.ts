...
export class Api {
  ...
  private buildHandlers(): Record<string, Handler> {
    ...
    return {
      ...
      "agent.skills.list": (p: { id: string }) => crew.store.listSkills(p.id),
      "agent.skills.set": (p: { id: string; name: string; content: string }) => { const s = crew.store.writeSkill(p.id, p.name, p.content); crew.bus.emit("agent.updated", crew.getAgent(p.id)); return s; },
      "agent.skills.delete": (p: { id: string; name: string }) => { crew.store.deleteSkill(p.id, p.name); crew.bus.emit("agent.updated", crew.getAgent(p.id)); return null; },
