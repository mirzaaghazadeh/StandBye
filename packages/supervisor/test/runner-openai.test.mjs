// Failure classification in the OpenAI-compatible runner: whatever the provider throws must
// come back as a classified RunnerOutput.failure kind (auth, credit, rate_limit, ...), never
// as a generic crash. Only auth and credit are owner-actionable — that is what blocksAgent()
// says; executeRun does not consume out.failure yet, so nothing pauses the agent today and
// the scheduler just re-fails it on the next check-in. The owner reviewed wiring the pause
// into executeRun (2026-09-03) and decided to keep it as-is: classification only, no pause.
//
